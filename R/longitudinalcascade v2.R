...
longitudinalcascade <- function(events.long,stages.order,groups.order=NA,
      ...
      # Generate time from each stage to death, censorship, and end of data collection
        for (i in 1:(length(stages.order))){
          ...
          if (!is.na(censorship.indicator) & !is.na(death.indicator)){
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- NA
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- ifelse(
              (events.wide[[paste("time.stage.",i,".to.censorship",sep="")]] < events.wide[[paste("time.stage.",i,".to.death",sep="")]]),1,0)
            events.wide[[paste("time.stage.",i,".to.death",sep="")]] <- ifelse(
              is.na(events.wide[[paste("time.stage.",i,".to.death",sep="")]]),events.wide[[paste("time.stage.",i,".to.censorship",sep="")]],events.wide[[paste("time.stage.",i,".to.death",sep="")]])
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- ifelse(
              (events.wide[[paste("time.stage.",i,".to.censorship",sep="")]] == events.wide[[paste("time.stage.",i,".to.death",sep="")]]),1,0)
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- ifelse(
              is.na(events.wide[[paste("time.stage.",i,".to.censorship",sep="")]]) & !is.na(events.wide[[paste("time.stage.",i,".to.death",sep="")]]) & is.na(events.wide[[paste("censorship.stage.",i,".to.death",sep="")]]),1,0)
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- ifelse(
              is.na(events.wide[[paste("time.stage.",i,".to.death",sep="")]]) ,NA,events.wide[[paste("censorship.stage.",i,".to.death",sep="")]])
          } else {
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- NA
            events.wide[[paste("censorship.stage.",i,".to.death",sep="")]] <- ifelse(
              !is.na(events.wide[[paste("time.stage.",i,".to.censorship",sep="")]]),1,0)
          }
        }
    }
    ...
    # Generate full survival dataset
    {
      # Main events
      {
        # Function to generate survival data for a single curve
          ...
          # Select data
            # Remove missing values for starting stages
              events.wide$selection <- !is.na(events.wide[[paste("time.stage.",start.stage.index,".to.",end.stage.index,sep="")]])
         ...
         # Function for determining survival stats reported
            surv.data.stats <- function(chart.time,chart.event,type){
              ...
              surv.data$event.type <- type
              return(surv.data)
            }
           # Generate list of events for main transitions
            ...
            surv.data <- surv.data.stats(chart.time,chart.event,"Main stage event")
            if (death==TRUE) {
              # Runs once only for n+1 transitions due to differing qualifications
              if (end.stage.index == start.stage.index+1){
                gen.mort <- function(i) {
                  # Get original times
                    chart.time <- as.integer(events.wide[[paste("time.death.stage.",i,".to.",(i+1),sep="")]][events.wide$selection==TRUE])
                    chart.event <- as.integer(!events.wide[[paste("censorship.death.stage.",i,".to.",(i+1),sep="")]][events.wide$selection==TRUE])
                  # Modify them so that NAs (previously indicated not qualified for measurement because before stage start) are now qualifiable events (to change denominators)
                    chart.event[is.na(chart.time)] <- 1
                    chart.time[is.na(chart.time)] <- time.horizon*1.1
                  # Get survival stats
                    surv.data <- surv.data.stats(chart.time,chart.event,"Death")
                    surv.data$reference.stage.index <- i
                  # Return data
                  return(surv.data)
                }
                surv.data <- do.call("rbind",lapply(start.stage.index:(length(stages.order)-1),function(x) gen.mort(x)))
                # Add post-final stage
                  # Get original times
                    chart.time <- as.integer(events.wide[[paste("time.stage.",i,".to.death",sep="")]][events.wide$selection==TRUE])
                    chart.event <- as.integer(!events.wide[[paste("censorship.stage.",i,".to.death",sep="")]][events.wide$selection==TRUE])
                  # Modify them so that NAs (previously indicated not qualified for measurement because before stage start) are now qualifiable events (to change denominators)
                    chart.event[is.na(chart.time)] <- 1
                    chart.time[is.na(chart.time)] <- time.horizon*1.1
                  # Get survival stats
                    surv.data.final <- surv.data.stats(chart.time,chart.event,"Death")
                    surv.data.final$reference.stage.index <- length(stages.order)
                  # Return data
                    surv.data <- rbind(surv.data,surv.data.final)
              } else {
                surv.data <- surv.data[0,]
              }
            }
          # Return
            return(surv.data)
  ...
  # Generate chart graphics
  {
    if (nochart==FALSE){
          ...
          if (is.na(death.indicator)==FALSE){
            ...
            # Create substage mortality dataset
              if (allow.sub.stage.mortality==TRUE){
                gen.substage.mort.df <- function(start.stage.index,start.stage.factor,end.stage.factor,reference.stage.index,group.factor){
                  #print(reference.stage.index)
                  reference <- surv.combined.chart[surv.combined.chart$start.stage.index==start.stage.index & surv.combined.chart$end.stage.index==reference.stage.index & surv.combined.chart$group.factor==group.factor,]
                  input <- surv.death.combined.chart[surv.death.combined.chart$start.stage.index==start.stage.index & surv.death.combined.chart$end.stage.factor==end.stage.factor & surv.death.combined.chart$reference.stage.index==reference.stage.index & surv.death.combined.chart$group.factor==group.factor,]
                  ...
                  df$group.factor <- group.factor
                  df$reference.stage.index <- reference.stage.index
                  return(df)
                }
                df.substage.stages <- unique(surv.death.combined.chart[ surv.death.combined.chart$end.stage.index > surv.death.combined.chart$start.stage.index,][c("start.stage.index","start.stage.factor","end.stage.factor","reference.stage.index","group.factor")])
                df.substage.stages <- df.substage.stages[df.substage.stages$start.stage.index!=df.substage.stages$reference.stage.index,]
                df.substage.death <- do.call("rbind",lapply(1:nrow(df.substage.stages),function(x) gen.substage.mort.df(df.substage.stages$start.stage.index[x],df.substage.stages$start.stage.factor[x],df.substage.stages$end.stage.factor[x],df.substage.stages$reference.stage.index[x],df.substage.stages$group.factor[x])))
      ...
      # Add main death event if indicated
        if (is.na(death.indicator)==FALSE){
          chart <- chart +
            geom_stepribbon(data=subset(surv.death.combined.chart,(surv.death.combined.chart$start.stage.index==(surv.death.combined.chart$end.stage.index)-1) & surv.death.combined.chart$reference.stage.index==surv.death.combined.chart$start.stage.index),
                            aes(x=.data$surv.time,ymin=1-.data$surv.p,ymax=1),
                                  alpha=1,fill=death.fill.color) +
            ggplot2::geom_step(data=subset(surv.death.combined.chart,(surv.death.combined.chart$start.stage.index==(surv.death.combined.chart$end.stage.index)-1) & surv.death.combined.chart$reference.stage.index==surv.death.combined.chart$start.stage.index),
          ...
          chart <- chart +
            geom_stepribbon(data=df.substage.death,
                            aes(x=.data$surv.time,ymin=.data$surv.p,ymax=surv.p.reference,
                                group=.data$reference.stage.index),
                            fill=death.fill.color,alpha=1)
            #scale_fill_discrete(c(rep(death.fill.color,5))) +
            # geom_step(data=df.substage.death,
            #                 aes(x=.data$surv.time,y=.data$surv.p,fill=.data$end.stage.factor),
            #                       alpha=1)
        } else {}
  ...
  # Prepare export data
  {
    ...
    if (!is.na(death.indicator)) { output.df <- c(output.df,
                      list("surv.death.combined" = surv.death.combined,"surv.death.combined.chart" = surv.death.combined.chart))
    } else {}
    ...
    if (allow.sub.stage.mortality!=FALSE) { output.df <- c(output.df,list("substage.death" = df.substage.death)) } else {}
